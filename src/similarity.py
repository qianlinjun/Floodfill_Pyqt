...
def graph_edit_distance(G1, G2, node_match=None, edge_match=None,
    ...
    best_edge_path = None
    for vertex_path, edge_path, cost, Cv, Ce in \
    ...
    return bestcost, best_vertex_path, best_edge_path, Cv, Ce
...
def optimize_edit_paths(G1, G2, node_match=None, edge_match=None,
    ...
    if node_del_cost:
        ...
        # new
        del_costs = C[0:m, 0:n].min(axis=1)*0.9 # m 返回每行最小值
    else:
    ...
    if node_ins_cost:
        ...
        # new
        ins_costs = C[0:m, 0:n].min(axis=0)*0.9 # n 返回每列最小值
    else:
    ...
    for vertex_path, edge_path, cost in \
        ...
        yield list(vertex_path), list(edge_path), cost, Cv, Ce
